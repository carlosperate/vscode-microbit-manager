...
describe('the two ways pairing can not produce a device', () => {
	 ...
	 * gain from trying.
	 */
	it('does not reach for a chooser this attempt may not open', async () => {
	...
	/**
	 * A refusal to pair is either a host with no bridge or a background attempt
	 * that must not ask, and only the caller knows which. Saying "this host
	 * cannot pair" here contradicts the caller's own startup line on every silent
	 * reconnect, in the one place anybody looks to find out what went wrong.
	 */
	it('does not blame the host for a refusal it cannot tell the reason for', async () => {
		const { spies, deps } = attempt({ canPair: () => false });

		await connectToBoard(deps);
		const logged = spies.log.mock.calls.map(([message]) => message).join('\n');
		expect(logged).not.toMatch(/host/i);
		expect(logged).toMatch(/cannot ask for permission/);
	});
