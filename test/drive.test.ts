...
import { boardAt, findBoards, matches, type DriveIo, type Machine, type Volume } from '../src/drive/volume';
...
describe('finding a mounted micro:bit', () => {
	...
	/** Every DAPLink build for a micro:bit writes one, so a volume without it is not a board. */
	it('refuses a volume that has no DETAILS.TXT at all', async () => {
		const io = fakeIo({ '/Volumes': ['MICROBIT'] }, { '/Volumes/MICROBIT/MICROBIT.HTM': '<html></html>' });

		expect(await findBoards(io, mac)).toEqual([]);
	});

	/** A board whose version is unreadable still flashes, from a hex for every board. */
	...
	/** A renamed stick gets one read, and it is the one that says this is not a board. */
	it('opens only DETAILS.TXT on a volume that is named MICROBIT', async () => {
		const io = fakeIo({ '/Volumes': ['MICROBIT'] }, { '/Volumes/MICROBIT/holiday-photos.zip': 'not a board' });

		expect(await findBoards(io, mac)).toEqual([]);
		expect(io.opened).toEqual(['/Volumes/MICROBIT/DETAILS.TXT']);
	});
...
/**
 * Asked immediately before bytes are written, against a drive found again rather
 * than remembered: a mount point outlives the board that made it, so the next
 * board to be plugged in takes the name the last one had.
 */
describe('recognising the board a hex was built for', () => {
	const built = { version: 'V2', serialNumber: '/Volumes/MICROBIT' } as const;

	it('takes the drive that is still where it was', () => {
		expect(matches({ path: '/Volumes/MICROBIT', version: 'V2' }, built)).toBe(true);
	});

	it('refuses the other board of two, which has its own mount point', () => {
		expect(matches({ path: '/Volumes/MICROBIT 1', version: 'V2' }, built)).toBe(false);
	});

	/** The one a version check is for: same name, same place, different board. */
	it('refuses a board of another version that took the same mount point', () => {
		expect(matches({ path: '/Volumes/MICROBIT', version: 'V1' }, built)).toBe(false);
	});

	/**
	 * Some builds ship no DETAILS.TXT, and refusing to flash them would make the
	 * drive route useless on exactly the boards that have no other route.
	 */
	it('accepts a drive whose version could not be read', () => {
		expect(matches({ path: '/Volumes/MICROBIT', version: undefined }, built)).toBe(true);
	});

	it('falls back to the version where there is nothing to identify the board with', () => {
		const unknown = { version: 'V2', serialNumber: undefined } as const;
		expect(matches({ path: '/Volumes/MICROBIT', version: 'V2' }, unknown)).toBe(true);
		expect(matches({ path: '/Volumes/MICROBIT', version: 'V1' }, unknown)).toBe(false);
	});
});
