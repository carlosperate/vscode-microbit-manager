...
export interface Attempt {
	...
	/**
	 * Whether **this attempt** may open a chooser. Two things say no: a host with
	 * no bridge, which desktop VS Code is, and a background attempt, which must
	 * never put a chooser in front of somebody who did not ask for one.
	 */
	canPair: () => boolean;
	/**
...
export async function connectToBoard(attempt: Attempt): Promise<Outcome> {
	...
	if (!attempt.canPair()) {
		// Which of the two reasons applies is the caller's to know: it logs the host's
		// own answer once at startup, and saying it again here would contradict it.
		attempt.log('No micro:bit is authorised, and this attempt cannot ask for permission');
